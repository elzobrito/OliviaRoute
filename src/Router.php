...
class Router
{
    ...
    public function __construct(?RouterConfig $config = null)
    {
        $this->route = [];
        $this->clauses = [];
        $this->config = $config ?? RouterConfig::fromGlobals();
        $this->trie = new Trie();
    }
    ...
    {
        $config = $this->resolveConfigForExecution($request_data);
        $routerDispatcher = new RouterDispatcher($this->route, $config, $this->trie, $this->clauses);
    ...
    private function resolveConfigForExecution($requestData): RouterConfig
    {
        if ($requestData instanceof Request) {
            return RouterConfig::fromRequest($requestData);
        }

        if (is_array($requestData)) {
            return RouterConfig::fromRequestData($requestData);
        }
